...
var Blue = Hero.extend({
	init: function(x, y, level) {
		...
		this.setSize(80, 80);
		this.cooldown = 0;
		this.setBlueState(blue_states.normal);
		...
		if (this.blueState === blue_states.normal)
		this.setImage(images.spritesb, 81, 0);
		else
		this.setImage(images.spritesbf, 81, 0);
		this.crouching = false;
		this.fast = false;
	},
	setBlueState: function(state) {
	...
	setPosition: function(x, y) {
        if(you=="blue"){socket.emit("blue1",salon+"&"+this.x)}
		if(you=="blue"){socket.emit("blue2",salon+"&"+this.y)}
		if(you=="blue"){socket.emit("blue5",salon+"&"+this.crouching)}
		if(you!="blue"){this._super(blue1, blue2);}
		if(you=="blue"){this._super(x, y);}
		var r = this.level.width - 640;
		// var r2 = this.level.height - 640;
		if (cam==4){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
// 		if (cam==1 && counterplayer==2){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
        levelheight = 32* 6
		if (cam==4) {ley = this.y >= levelheight?-this.y+levelheight:0}
// 		if (cam==1 && counterplayer==2) {ley = this.y >= levelheight?-this.y+levelheight:0}
		...
		if (this.x >= definedLevels[levelnumber].x && this.x <= definedLevels[levelnumber].x + marge && this.y >= definedLevels[levelnumber].y && this.y <= definedLevels[levelnumber].y + 128 && this.onground)
            ...
            if(d5==1) this.die()
        if(c52=="hurt" && you!="blue") this.hurt()
        if(c5=="grow"  && you!="blue") this.grow()
        if(s5=="true"  && you!="blue") this.shooter()
		//setInterval("i++;ghost123[i]={x:"+this.x+",y:"+this.y+"}",1000)
			 // alert(x+","+y)
	},
	input: function(keys) {
        if(you=="blue"){this.crouching = keysb.down;}
        if(you!="blue"){this.crouching = blue5;}
		...
		if(!this.crouching) {
			...
			if(keys.accelerate && this.blueState === blue_states.fire)
				this.shoot();
	...
	victory: function() {
		...
		this.view.show();
		if (this.blueState === blue_states.normal)
		this.setImage(images.spritesb, this.state === size_states.small ? 241 : 161, 81);
		else
		this.setImage(images.spritesbf, this.state === size_states.small ? 241 : 161, 81);
		this.level.next();
	...
	shooter: function() {
        ...
        socket.emit("s5",salon+"&"+true)
		this.setBlueState(blue_states.fire);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, true, 'WalkRightSmall'))
				this.setImage(images.spritesb, 0, 0);
		} else {
			if(!this.setupFrames(9, 2, true, 'WalkRightBig'))
				if (this.blueState === blue_states.normal)
				this.setImage(images.spritesb, 0, 243);
				else
				this.setImage(images.spritesbf, 0, 243);
		}
	},
	walkLeft: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, false, 'WalkLeftSmall'))
				this.setImage(images.spritesb, 80, 81);
		} else {
			if(!this.setupFrames(9, 2, false, 'WalkLeftBig'))
				if (this.blueState === blue_states.normal)
				this.setImage(images.spritesb, 81, 162);
				else
				this.setImage(images.spritesbf, 81, 162);
		}
	},
	stand: function() {
		var coords = this.standSprites[this.state - 1][this.direction === directions.left ? 0 : 1][this.onground ? 0 : 1];
		if (this.blueState === blue_states.normal)
		this.setImage(images.spritesb, coords.x, coords.y);
		else
		this.setImage(images.spritesbf, coords.x, coords.y);
		this.clearFrames();
	},
	crouch: function() {
