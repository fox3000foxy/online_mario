...
var Indigo = Hero.extend({
	init: function(x, y, level) {
		...
		this.setSize(80, 80);
		this.cooldown = 0;
		this.setIndigoState(indigo_states.normal);
		...
		if (this.indigoState === indigo_states.normal)
		this.setImage(images.spritesi, 81, 0);
		else
		this.setImage(images.spritesif, 81, 0);
		this.crouching = false;
		this.fast = false;
	},
	setIndigoState: function(state) {
		this.indigoState = state;
	},
	...
	setPosition: function(x, y) {
        if(you=="indigo"){socket.emit("indigo1",salon+"&"+this.x)}
		if(you=="indigo"){socket.emit("indigo2",salon+"&"+this.y)}
		if(you=="indigo"){socket.emit("indigo5",salon+"&"+this.crouching)}
		if(you!="indigo"){this._super(indigo1, indigo2);}
		if(you=="indigo"){this._super(x, y);}
		var r = this.level.width - 640;
		// var r2 = this.level.height - 640;
		if (cam==6){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
// 		if (cam==1 && counterplayer==2){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
        levelheight = 32* 6
		if (cam==6) {ley = this.y >= levelheight?-this.y+levelheight:0}
// 		if (cam==1 && counterplayer==2) {ley = this.y >= levelheight?-this.y+levelheight:0}
		...
		if (this.x >= definedLevels[levelnumber].x && this.x <= definedLevels[levelnumber].x + marge && this.y >= definedLevels[levelnumber].y && this.y <= definedLevels[levelnumber].y + 128 && this.onground)
            ...
            if(d7==1) this.die()
        if(c72=="hurt" && you!="indigo") this.hurt()
        if(c7=="grow"  && you!="indigo") this.grow()
        if(s7=="true"  && you!="indigo") this.shooter()
		//setInterval("i++;ghost123[i]={x:"+this.x+",y:"+this.y+"}",1000)
			 // alert(x+","+y)
	},
	input: function(keys) {
        if(you=="indigo"){this.crouching = keysi.down;}
        if(you!="indigo"){this.crouching = indigo5;}
		...
		if(!this.crouching) {
			...
			if(keys.accelerate && this.indigoState === indigo_states.fire)
				this.shoot();
	...
	victory: function() {
		...
		this.view.show();
		if (this.indigoState === indigo_states.normal)
		this.setImage(images.spritesi, this.state === size_states.small ? 241 : 161, 81);
		else
		this.setImage(images.spritesif, this.state === size_states.small ? 241 : 161, 81);
		this.level.next();
	...
	shooter: function() {
        ...
        socket.emit("s7",salon+"&"+true)
		this.setIndigoState(indigo_states.fire);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, true, 'WalkRightSmall'))
				this.setImage(images.spritesi, 0, 0);
		} else {
			if(!this.setupFrames(9, 2, true, 'WalkRightBig'))
				if (this.indigoState === indigo_states.normal)
				this.setImage(images.spritesi, 0, 243);
				else
				this.setImage(images.spritesif, 0, 243);
		}
	},
	walkLeft: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, false, 'WalkLeftSmall'))
				this.setImage(images.spritesi, 80, 81);
		} else {
			if(!this.setupFrames(9, 2, false, 'WalkLeftBig'))
				if (this.indigoState === indigo_states.normal)
				this.setImage(images.spritesi, 81, 162);
				else
				this.setImage(images.spritesif, 81, 162);
		}
	},
	stand: function() {
		var coords = this.standSprites[this.state - 1][this.direction === directions.left ? 0 : 1][this.onground ? 0 : 1];
		if (this.indigoState === indigo_states.normal)
		this.setImage(images.spritesi, coords.x, coords.y);
		else
		this.setImage(images.spritesif, coords.x, coords.y);
		this.clearFrames();
	},
	crouch: function() {
