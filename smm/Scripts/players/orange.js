...
var Orange = Hero.extend({
	init: function(x, y, level) {
		...
		this.setSize(80, 80);
		this.cooldown = 0;
		this.setOrangeState(orange_states.normal);
		...
		if (this.orangeState === orange_states.normal)
		this.setImage(images.spriteso, 81, 0);
		else
		this.setImage(images.spritesof, 81, 0);
		this.crouching = false;
		this.fast = false;
	},
	setOrangeState: function(state) {
		this.orangeState = state;
	},
	...
	setPosition: function(x, y) {
        if(you=="orange"){socket.emit("orange1",salon+"&"+this.x)}
		if(you=="orange"){socket.emit("orange2",salon+"&"+this.y)}
		if(you=="orange"){socket.emit("orange5",salon+"&"+this.crouching)}
		if(you!="orange"){this._super(orange1, orange2);}
		if(you=="orange"){this._super(x, y);}
		var r = this.level.width - 640;
		// var r2 = this.level.height - 640;
		if (cam==3){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
// 		if (cam==1 && counterplayer==2){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
        levelheight = 32* 6
		if (cam==3) {ley = this.y >= levelheight?-this.y+levelheight:0}
// 		if (cam==1 && counterplayer==2) {ley = this.y >= levelheight?-this.y+levelheight:0}
		...
		if (this.x >= definedLevels[levelnumber].x && this.x <= definedLevels[levelnumber].x + marge && this.y >= definedLevels[levelnumber].y && this.y <= definedLevels[levelnumber].y + 128 && this.onground)
            ...
            if(d4==1) this.die()
        if(c42=="hurt" && you!="orange") this.hurt()
        if(c4=="grow"  && you!="orange") this.grow()
        if(s4=="true"  && you!="orange") this.shooter()
		//setInterval("i++;ghost123[i]={x:"+this.x+",y:"+this.y+"}",1000)
			 // alert(x+","+y)
	},
	input: function(keys) {
        if(you=="orange"){this.crouching = keyso.down;}
        if(you!="orange"){this.crouching = orange5;}
		...
		if(!this.crouching) {
			...
			if(keys.accelerate && this.orangeState === orange_states.fire)
				this.shoot();
	...
	victory: function() {
		...
		this.view.show();
		if (this.orangeState === orange_states.normal)
		this.setImage(images.spriteso, this.state === size_states.small ? 241 : 161, 81);
		else
		this.setImage(images.spritesof, this.state === size_states.small ? 241 : 161, 81);
		this.level.next();
	...
	shooter: function() {
        ...
        socket.emit("s4",salon+"&"+true)
		this.setOrangeState(orange_states.fire);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, true, 'WalkRightSmall'))
				this.setImage(images.spriteso, 0, 0);
		} else {
			if(!this.setupFrames(9, 2, true, 'WalkRightBig'))
				if (this.orangeState === orange_states.normal)
				this.setImage(images.spriteso, 0, 243);
				else
				this.setImage(images.spritesof, 0, 243);
		}
	},
	walkLeft: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, false, 'WalkLeftSmall'))
				this.setImage(images.spriteso, 80, 81);
		} else {
			if(!this.setupFrames(9, 2, false, 'WalkLeftBig'))
				if (this.orangeState === orange_states.normal)
				this.setImage(images.spriteso, 81, 162);
				else
				this.setImage(images.spritesof, 81, 162);
		}
	},
	stand: function() {
		var coords = this.standSprites[this.state - 1][this.direction === directions.left ? 0 : 1][this.onground ? 0 : 1];
		if (this.orangeState === orange_states.normal)
		this.setImage(images.spriteso, coords.x, coords.y);
		else
		this.setImage(images.spritesof, coords.x, coords.y);
		this.clearFrames();
	},
	crouch: function() {
