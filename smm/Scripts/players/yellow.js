...
var Yellow = Hero.extend({
	init: function(x, y, level) {
		...
		this.setSize(80, 80);
		this.cooldown = 0;
		this.setYellowState(yellow_states.normal);
		...
		if (this.yellowState === yellow_states.normal)
		this.setImage(images.spritesy, 81, 0);
		else
		this.setImage(images.spritesyf, 81, 0);
		this.crouching = false;
		this.fast = false;
	},
	setYellowState: function(state) {
	...
	setPosition: function(x, y) {
        if(you=="yellow"){socket.emit("yellow1",salon+"&"+this.x)}
		if(you=="yellow"){socket.emit("yellow2",salon+"&"+this.y)}
		if(you=="yellow"){socket.emit("yellow5",salon+"&"+this.crouching)}
		if(you!="yellow"){this._super(yellow1, yellow2);}
		if(you=="yellow"){this._super(x, y);}
		var r = this.level.width - 640;
		// var r2 = this.level.height - 640;
		if (cam==8){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
// 		if (cam==1 && counterplayer==2){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
        levelheight = 32* 6
		if (cam==8) {ley = this.y >= levelheight?-this.y+levelheight:0}
// 		if (cam==1 && counterplayer==2) {ley = this.y >= levelheight?-this.y+levelheight:0}
		...
		if (this.x >= definedLevels[levelnumber].x && this.x <= definedLevels[levelnumber].x + marge && this.y >= definedLevels[levelnumber].y && this.y <= definedLevels[levelnumber].y + 128 && this.onground)
            ...
            if(d9==1) this.die()
        if(c92=="hurt" && you!="yellow") this.hurt()
        if(c9=="grow"  && you!="yellow") this.grow()
        if(s9=="true"  && you!="yellow") this.shooter()
		//setInterval("i++;ghost123[i]={x:"+this.x+",y:"+this.y+"}",1000)
			 // alert(x+","+y)
	},
	input: function(keys) {
        if(you=="yellow"){this.crouching = keysy.down;}
        if(you!="yellow"){this.crouching = yellow5;}
		...
		if(!this.crouching) {
			...
			if(keys.accelerate && this.yellowState === yellow_states.fire)
				this.shoot();
	...
	victory: function() {
		...
		this.view.show();
		if (this.yellowState === yellow_states.normal)
		this.setImage(images.spritesy, this.state === size_states.small ? 241 : 161, 81);
		else
		this.setImage(images.spritesyf, this.state === size_states.small ? 241 : 161, 81);
		this.level.next();
	...
	shooter: function() {
        ...
        socket.emit("s9",salon+"&"+true)
		this.setYellowState(yellow_states.fire);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, true, 'WalkRightSmall'))
				this.setImage(images.spritesy, 0, 0);
		} else {
			if(!this.setupFrames(9, 2, true, 'WalkRightBig'))
				if (this.yellowState === yellow_states.normal)
				this.setImage(images.spritesy, 0, 243);
				else
				this.setImage(images.spritesyf, 0, 243);
		}
	},
	walkLeft: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, false, 'WalkLeftSmall'))
				this.setImage(images.spritesy, 80, 81);
		} else {
			if(!this.setupFrames(9, 2, false, 'WalkLeftBig'))
				if (this.yellowState === yellow_states.normal)
				this.setImage(images.spritesy, 81, 162);
				else
				this.setImage(images.spritesyf, 81, 162);
		}
	},
	stand: function() {
		var coords = this.standSprites[this.state - 1][this.direction === directions.left ? 0 : 1][this.onground ? 0 : 1];
		if (this.yellowState === yellow_states.normal)
		this.setImage(images.spritesy, coords.x, coords.y);
		else
		this.setImage(images.spritesyf, coords.x, coords.y);
		this.clearFrames();
	},
	crouch: function() {
