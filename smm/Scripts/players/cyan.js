...
var Cyan = Hero.extend({
	init: function(x, y, level) {
		...
		this.setSize(80, 80);
		this.cooldown = 0;
		this.setCyanState(cyan_states.normal);
		...
		if (this.cyanState === cyan_states.normal)
		this.setImage(images.spritesc, 81, 0);
		else
		this.setImage(images.spritescf, 81, 0);
		this.crouching = false;
		this.fast = false;
	},
	setCyanState: function(state) {
		this.cyanState = state;
	},
	...
	setPosition: function(x, y) {
        if(you=="cyan"){socket.emit("cyan1",salon+"&"+this.x)}
		if(you=="cyan"){socket.emit("cyan2",salon+"&"+this.y)}
		if(you=="cyan"){socket.emit("cyan5",salon+"&"+this.crouching)}
		if(you!="cyan"){this._super(cyan1, cyan2)}
		if(you=="cyan"){this._super(x, y);}
		var r = this.level.width - 640;
		// var r2 = this.level.height - 640;
		if (cam==5){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
// 		if (cam==1 && counterplayer==2){w = (this.x <= 210) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));}
        levelheight = 32* 6
		if (cam==5) {ley = this.y >= levelheight?-this.y+levelheight:0}
// 		if (cam==1 && counterplayer==2) {ley = this.y >= levelheight?-this.y+levelheight:0}
		...
		if (this.x >= definedLevels[levelnumber].x && this.x <= definedLevels[levelnumber].x + marge && this.y >= definedLevels[levelnumber].y && this.y <= definedLevels[levelnumber].y + 128 && this.onground)
            ...
            if(d6==1) this.die()
        if(c62=="hurt" && you!="cyan") this.hurt()
        if(c6=="grow"  && you!="cyan") this.grow()
        if(s6=="true"  && you!="cyan") this.shooter()
		//setInterval("i++;ghost123[i]={x:"+this.x+",y:"+this.y+"}",1000)
			 // alert(x+","+y)
	},
	input: function(keys) {
        if(you=="cyan"){this.crouching = keysc.down;}
        if(you!="cyan"){this.crouching = cyan5}
		...
		if(!this.crouching) {
			...
			if(keys.accelerate && this.cyanState === cyan_states.fire)
				this.shoot();
	...
	victory: function() {
		...
		this.view.show();
		if (this.cyanState === cyan_states.normal)
		this.setImage(images.spritesc, this.state === size_states.small ? 241 : 161, 81);
		else
		this.setImage(images.spritescf, this.state === size_states.small ? 241 : 161, 81);
		this.level.next();
	...
	shooter: function() {
        ...
        socket.emit("s6",salon+"&"+true)
		this.setCyanState(cyan_states.fire);
	},
	...
	walkRight: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, true, 'WalkRightSmall'))
				this.setImage(images.spritesc, 0, 0);
		} else {
			if(!this.setupFrames(9, 2, true, 'WalkRightBig'))
				if (this.cyanState === cyan_states.normal)
				this.setImage(images.spritesc, 0, 243);
				else
				this.setImage(images.spritescf, 0, 243);
		}
	},
	walkLeft: function() {
		if(this.state === size_states.small) {
			if(!this.setupFrames(8, 2, false, 'WalkLeftSmall'))
				this.setImage(images.spritesc, 80, 81);
		} else {
			if(!this.setupFrames(9, 2, false, 'WalkLeftBig'))
				if (this.cyanState === cyan_states.normal)
				this.setImage(images.spritesc, 81, 162);
				else
				this.setImage(images.spritescf, 81, 162);
		}
	},
	stand: function() {
		var coords = this.standSprites[this.state - 1][this.direction === directions.left ? 0 : 1][this.onground ? 0 : 1];
		if (this.cyanState === cyan_states.normal)
		this.setImage(images.spritesc, coords.x, coords.y);
		else
		this.setImage(images.spritescf, coords.x, coords.y);
		this.clearFrames();
	},
	crouch: function() {
